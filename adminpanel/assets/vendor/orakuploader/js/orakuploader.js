(function ( $ ) {
    ...
    $.fn.orakuploader = function( options ) {
		...
		var settings = $.extend({
			...
			orakuploader_resize_width        : '',
			orakuploader_resize_height       : '',
			orakuploader_thumbnail_size      : '',
			orakuploader_thumbnail_width     : '',
			orakuploader_thumbnail_height    : '',
			...
			orakuploader_base_url            : "",
			orakuploader_image_url           : "",
			orakuploader_image_view_modal    : ".image-viewer",
		...
		if($(this).attr('orakuploader') == 'on') {
			...
			holdername = $(this).replaceWith(getHtml($(this).attr('id'), settings.orakuploader_add_image, settings.orakuploader_add_label, settings.orakuploader_path,settings));
		...
		for (i = 0; i < settings.orakuploader_attach_images.length; i++) {  
			...
			$(clone).html("<div class='picture_delete'>"+settings.orakuploader_file_delete_label+"</div><div class='picture_view' src='"+settings.orakuploader_image_url+""+settings.orakuploader_main_path+"/"+image+"'></div><img src='"+settings.orakuploader_image_url+""+settings.orakuploader_thumbnail_path+"/"+image+"' alt='' onerror=this.src='"+settings.orakuploader_base_url+"assets/images/uploader/no-image.jpg' class='picture_uploaded'/> <input type='hidden' value='"+image+"' name='"+settings.orakuploader_field_name+"[]' />");
			$(clone).attr('id', image);
		...
		$(holdername).on("click", ".picture_view", function() {
	
		var pathOfViewFile=$(this).attr("src");	
		$(settings.orakuploader_image_view_modal).modal();
		$(settings.orakuploader_image_view_modal).find(".modal-body").html("<img src='"+pathOfViewFile+"?"+ new Date().getTime()+"' style='    max-width: 100%;height: auto;' />");
		});
		...
		$(holdername).on("click", ".rotate_picture", function() {	
			...
			$.ajax({
				url: settings.orakuploader_path+"?rotate="+encodeURIComponent($(this).parent().attr('filename'))+"&degree_lvl="+$(this).closest('.rotate_picture').attr('degree-lvl')+"&path="+settings.orakuploader_path+"&resize_to="+settings.orakuploader_resize_to+"&thumbnail_size="+settings.orakuploader_thumbnail_size+"&main_path="+settings.orakuploader_main_path+"&thumbnail_path="+settings.orakuploader_thumbnail_path
	...
	function upload(file, clone, place, holder, settings)
	{
		...
		xhr.onreadystatechange = function() 
		{
			...
			if (xhr.readyState == 4) 
			{
				...
				$(clone).html("<div class='picture_delete'>"+settings.orakuploader_file_delete_label+"</div><div class='picture_view' src='"+settings.orakuploader_image_url+""+settings.orakuploader_main_path+"/"+xhr.responseText+"'></div>"+rotation_html+"<img src='"+settings.orakuploader_image_url+""+settings.orakuploader_thumbnail_path+"/"+xhr.responseText+"' alt='' onerror=this.src='"+settings.orakuploader_base_url+"assets/images/uploader/no-image.jpg' class='picture_uploaded'/> <input type='hidden' value='"+xhr.responseText+"' name='"+settings.orakuploader_field_name+"[]' />");
...
function getHtml(name, add_image, add_label, path,settings)
{
	return '<div id="'+name+'_to_clone" class="clone_item"><div class="multibox file"><div class="loading"><img src="'+settings.orakuploader_base_url+'assets/images/uploader/loader.gif" alt="loader"/></div></div></div>\
		<div id="'+name+'DDArea">\
			<div id="'+name+'">\
			</div>\
